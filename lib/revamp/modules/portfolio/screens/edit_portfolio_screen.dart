...
class EditPortfolioScreen extends StatefulWidget {
  ...
  const EditPortfolioScreen({
    ...
    this.feed,
    this.authBloc,
    this.feedBloc, this.onRefresh,
  }) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _EditPortfolioScreen();
  }
}

class _EditPortfolioScreen extends State<EditPortfolioScreen> {
    ...
    if (croppedFile != null) {
      ...
      portfolioBloc
          .uploadPortfolioPictures(
              context, [croppedFile], widget.feed.portfolioId)
          .then((response) {
        if (response.code == "success") {
          widget.feed.pictures = response.payload.pictures;
          setState(() {
            loadRemove = false;
          });
        } else {
  ...
  setPortfolio() {
    setState(() {
      ...
      descController.text = widget.feed.description;
      youtubeController.text = widget.feed.youtubeUrl;
      amountController.text = widget.feed.amount.toString();
      duration =
          widget.feed.duration > 0 ? widget.feed.duration.toDouble() : 1.0;
    });
  }

  setCategory() {
    for (Category ctg in categoryList) {
      if (ctg.category.contains(widget.feed.category)) {
        category = ctg;
  ...
  setSubCategory() {
    for (SubCategory subCtg in subCategoryList) {
      if (subCtg.subCategory.contains(widget.feed.subCategory)) {
        subCategory = subCtg;
        categoryBloc.requestGetTypeCategory(
          context,
          category.type,
  ...
  setTypeCategory() {
    for (TypeCategory typeCtg in typeCategoryList) {
      if (typeCtg.typeCategory.contains(widget.feed.typeCategory)) {
        typeCategory = typeCtg;
      }
    }
  }
        ...
        appBar: AppBar(
          ...
          actions: <Widget>[
            IconButton(
              ...
              onPressed: () {
                DialogConfirmDeletePortfolio(context, "edit", () {
                  portfolioBloc
                      .deletePortfolio(context, widget.feed.portfolioId)
                      .then((response) {
                    if (response.code == "success") {
                      SuccessScreenPortfolio(
                          context,
                          widget.authBloc,
                          Portfolio(
                            title: widget.feed.title,
                            createdAt: widget.feed.createdAt,
                            category: widget.feed.category,
                            typeCategory: widget.feed.typeCategory,
                          ),
                          "delete",
                          () {});
                    } else {
                  ...
                  Container(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        if (index < widget.feed.pictures.length) {
                          return GestureDetector(
                            onTap: () => ShowImagePreview(
                              context: context,
                              index: index,
                              pictures: widget.feed.pictures,
                            ),
                            child: CardGallery(
                                context: context,
                                size: size,
                                path: widget.feed.pictures[index].path,
                                isRemovable: true,
                                loadingRemove: loadRemove,
                                onRemove: () {
                                  setState(() {
                                    loadRemove = true;
                                  });
                                  widget.feedBloc
                                      .deletePicture(
                                          ...
                                          widget.feed.pictures[index].id)
                                      .then((response) {
                                    if (response.code == "success") {
                                      widget.feed.pictures.removeAt(index);
                                      widget.feedBloc.getPortfolioDetail(
                                          context, widget.feed.portfolioId);
                                      setState(() {
                                        loadRemove = false;
                                      });
                                    } else {
                        ...
                        } else if (index == widget.feed.pictures.length) {
...
//                        ),
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      children: <Widget>[
                        ...
                        GestureDetector(
                          onTap: () => DialogStudioInfo(
                              context, widget.authBloc, "portfolio"),
                          child: Container(
              ...
              StreamBuilder<UpdatePortfolioState>(
                  stream: portfolioBloc.getUpdateStatus,
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                        ...
                        onWidgetDidBuild(() {
                          if (snapshot.data.standby) {
                            showDialogMessage(context, snapshot.data.message,
                                "Ubah Portfolio gagal, silahkan coba lagi");
...
//                                imageGalleries);
                    ...
                    return Container(
                      ...
                      child: RaisedButtonText(
                        text: Texts.updatePortfolio.toUpperCase(),
                        color: Colors.black,
