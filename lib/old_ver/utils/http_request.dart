...
class HttpRequest {
  ...
  static Future<Map<String, dynamic>> get(
    ...
    if (useAuth == null || useAuth) {
      String jwt;
      await LocalStorage.get(LocalStorageKey.AuthToken).then((value) {
        jwt = value;
      });
  ...
  static Future<Map<String, dynamic>> post(
    ...
    if (useAuth == null || useAuth) {
      ...
      if(url == Url.sendResetPin){
        jwt = await LocalStorage.get(LocalStorageKey.PinToken);

        if (jwt == null) {
          throw Exception('jwt auth not found');
        }
      } else {
        jwt = await LocalStorage.get(LocalStorageKey.AuthToken);
  ...
  static Future<Map<String, dynamic>> put(
    ...
    try {
      response = await _httpClient
          ...
          headers: _headers, body: jsonEncode(bodyJson != null ? bodyJson : {}))
          .timeout(Duration(seconds: 3));
    } catch (err) {
      throw TimeoutException('Request Timeout');
    }
  ...
  static Future<Map<String, dynamic>> putFile({
    ...
    print("[LOG POST] Headers: " + request.headers.toString());
